...
impl PositionTrackingMetrics {
    ...
    pub fn generate_performance_report(&self) -> String {
        ...
        report.push_str(&format!("\n⏱️  Timing Statistics:\n"));
        report.push_str(&format!(
            "   Average Decode Time: {:.3}ms\n",
            self.get_average_decode_time_ms()
        ));
        report.push_str(&format!(
            "   Average Position Update Time: {:.3}ms\n",
            self.get_average_position_update_time_ms()
        ));
        report.push_str(&format!(
            "   Average Validation Time: {:.3}ms\n",
        ...
        if self.memory_usage_kb < 10_000 {
            // Less than 10MB
            report.push_str("   ✅ Memory usage: EXCELLENT (< 10MB)\n");
        } else if self.memory_usage_kb < 50_000 {
            // Less than 50MB
...
impl MonitoredPositionTracker {
    ...
    pub fn try_decode_move_with_monitoring(
        ...
        self.metrics
            .record_decode_attempt(result.is_ok(), decode_duration);

        result
    }

    pub fn apply_move_with_monitoring(
        &mut self,
        scid_move: &crate::position::ScidMove,
    ) -> Result<(), String> {
        let start_time = Instant::now();
        ...
        self.metrics
            .record_position_update(position_update_duration);
...
impl PositionTrackerStatsCompat {
    fn from_position(pos: &ScidPosition) -> Self {
        ...
        let total_moves = (full.saturating_sub(1)) * 2
            + if pos.to_move == crate::position::Color::Black {
                1
            } else {
                0
            };
