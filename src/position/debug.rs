...
use crate::position::{ScidPosition, Color, PieceType};
...
impl ScidPosition {
    ...
    pub fn debug_position_state(&self) -> String {
        ...
        result.push_str(&format!("=== POSITION STATE DEBUG ===\n"));
        ...
        if !self.move_history.is_empty() {
            ...
            for (i, mv) in self.move_history.iter().enumerate() {
                result.push_str(&format!("  {}: {} -> {} (piece {})\n", 
                    i + 1, mv.from.to_algebraic(), mv.to.to_algebraic(), mv.piece_num));
            }
        }
    ...
    pub fn validate_piece_lists(&self) -> Result<(), String> {
        for color in [Color::White, Color::Black] {
            ...
            for i in 0..self.piece_counts[color_idx] {
                if i >= 16 {
                    return Err(format!("Too many pieces for {}: {}", color.to_string(), self.piece_counts[color_idx]));
                }
                
                let square = self.piece_lists[color_idx][i];
                if square.0 >= 64 {
                    return Err(format!("Invalid square {} for {} piece {}", square.0, color.to_string(), i));
                }
                
                let piece_on_board = self.board[square.0 as usize];
                if piece_on_board == PieceType::Empty {
                    return Err(format!("Piece list shows {} piece {} at {} but board is empty", 
                        color.to_string(), i, square.to_algebraic()));
                }
                
                // Check reverse lookup
                if self.list_pos[square.0 as usize] != i as u8 {
                    return Err(format!("Reverse lookup mismatch at {}: expected {}, got {}", 
                        square.to_algebraic(), i, self.list_pos[square.0 as usize]));
                }
            }
        }
    ...
    pub fn is_starting_position(&self) -> bool {
        ...
        for (i, (expected_piece, expected_square)) in super::SCID_WHITE_PIECE_INIT.iter().enumerate() {
        ...
        for (i, (expected_piece, expected_square)) in super::SCID_BLACK_PIECE_INIT.iter().enumerate() {
        ...
        for color in [Color::White, Color::Black] {
            ...
            for i in 0..std::cmp::min(self.piece_counts[color_idx], other.piece_counts[color_idx]) {
                ...
                if before_square != after_square {
                    result.push_str(&format!("{} piece {}: {} -> {}\n", 
                        color.to_string(), i, before_square.to_algebraic(), after_square.to_algebraic()));
                }
            }
        }
