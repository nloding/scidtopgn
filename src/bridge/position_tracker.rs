...
impl ScidPositionTracker {
    ...
    /// Create a position tracker from an existing chess position
    pub fn from_position(position: Chess) -> Self {
        ...
        // Initialize piece lists from the given position
        tracker.rebuild_piece_lists();
        tracker
    }
    ...
    pub fn apply_scid_move(&mut self, scid_move: &DecodedMove) -> Result<Move> {
        ...
        if !self.current_position.is_legal(&shakmaty_move) {
            return Err(ScidError::conversion_error(format!(
                ...
                san,
                self.current_position.board().to_string()
            )));
        }
        ...
        self.current_position =
            self.current_position
                ...
                .play(&shakmaty_move)
                .map_err(|e| {
                    ScidError::conversion_error(format!("Failed to play move {}: {}", san, e))
                })?;
    ...
    fn convert_scid_to_shakmaty(&self, scid_move: &DecodedMove) -> Result<Move> {
        ...
        let chess_move =
            self.create_shakmaty_move(piece.role, from_square, to_square, scid_move)?;
    ...
    fn create_shakmaty_move(
        ...
        if role == Role::Pawn {
            ...
            if let Some(promotion_role) =
                self.get_pawn_promotion_from_move_value(scid_move.move_value)
            {
    ...
    fn check_castling_move(&self, from: Square, to: Square) -> Result<Option<Move>> {
        ...
        let home_square = if self.to_move == Color::White {
            Square::E1
        } else {
            Square::E8
        };
        ...
        // Determine castling type based on target square
        let (rook_home, rook_target) = if self.to_move == Color::White {
            (Square::H1, Square::F1) // Kingside: H1->F1
        } else {
            (Square::H8, Square::F8) // Kingside: H8->F8
        };
        
        let (queen_rook_home, queen_rook_target) = if self.to_move == Color::White {
            (Square::A1, Square::D1) // Queenside: A1->D1
        } else {
            (Square::A8, Square::D8) // Queenside: A8->D8
        };
        
        if to == rook_target {
            // Kingside castling
            if self.verify_castling_legality(from, rook_home, rook_target)? {
                return Ok(Some(Move::Castle {
                    king: to,
                    rook: rook_home,
                }));
            }
        } else if to == queen_rook_target {
            // Queenside castling
            if self.verify_castling_legality(from, queen_rook_home, queen_rook_target)? {
                return Ok(Some(Move::Castle {
                    king: to,
                    rook: queen_rook_home,
                }));
            }
        }
    ...
    /// Verify castling legality according to chess rules
    fn verify_castling_legality(&self, king_from: Square, rook_from: Square, rook_to: Square) -> Result<bool> {
        // Check if king and rook are on correct squares
        if king_from != if self.to_move == Color::White { Square::E1 } else { Square::E8 } {
            return Ok(false);
        }
        
        // Check if rook is on correct starting square
        let expected_rook_from = if rook_to == Square::F1 || rook_to == Square::F8 {
            Square::H1 // Kingside rook
        } else if rook_to == Square::D1 || rook_to == Square::D8 {
            Square::A1 // Queenside rook
        } else {
            return Ok(false);
        };
        
        if rook_from != expected_rook_from {
            return Ok(false);
        }
        ...
        if rook_piece.color != self.to_move || rook_piece.role != Role::Rook {
            return Ok(false);
        }
        
        // Check if path between king and rook is clear
        let king_file = king_from.file() as usize;
        let rook_file = rook_from.file() as usize;
        let rank = king_from.rank() as usize;
        
        let min_file = king_file.min(rook_file);
        let max_file = king_file.max(rook_file);
        ...
        // Check if king is currently in check (simplified check)
        // Full implementation would verify king doesn't pass through check
    ...
    fn is_king_in_check(&self) -> bool {
        ...
        // Full implementation would use shakmaty's position validation
        false
    }
    ...
    fn rebuild_piece_lists(&mut self) {
        ...
        for color in [Color::White, Color::Black] {
            ...
            // Ensure we have exactly 16 pieces (SCID maximum) by padding if necessary
            while pieces.len() < 16 {
                pieces.push(Square::A1); // Placeholder for unused piece numbers
            }
    ...
    pub fn scid_piece_list(&self, color: Color) -> Vec<Square> {
        ...
        if let Some(qs_rook) = rooks.get(0) {
            list.push(*qs_rook);
        }
        if let Some(ks_rook) = rooks.get(1) {
            list.push(*ks_rook);
        }
        ...
        if let Some(qs_bishop) = bishops.get(0) {
            list.push(*qs_bishop);
        }
        if let Some(ks_bishop) = bishops.get(1) {
            list.push(*ks_bishop);
        }
        ...
        if let Some(qs_knight) = knights.get(0) {
            list.push(*qs_knight);
        }
        if let Some(ks_knight) = knights.get(1) {
            list.push(*ks_knight);
        }
    ...
    pub fn scid_piece_lists(&self) -> (Vec<Square>, Vec<Square>) {
        (
            self.scid_piece_list(Color::White),
            self.scid_piece_list(Color::Black),
        )
    }
    ...
    pub fn decode_scid_move(&self, color: Color, move_byte: u8) -> Option<Move> {
        ...
        let candidate_moves: Vec<_> = legals
            .into_iter()
...
impl ScidPositionTracker {
        ...
        match move_value {
            ...
            1..=10 => {
                ...
                if target_square_i32 >= 0 && target_square_i32 < 64 {
                    ...
                    Err(ScidError::conversion_error(format!(
                        "King move out of bounds: {} + {} = {}",
                        from as u8, diff, target_square_i32
                    )))
                }
            }
            _ => Err(ScidError::conversion_error(format!(
                "Invalid king move value: {}",
                move_value
            ))),
        }
    }
        ...
        if move_value >= 8 {
            ...
            if target_rank < 8 {
                Ok(Square::from_coords(
                    from.file(),
                    Rank::new(target_rank as u32),
                ))
            } else {
                Err(ScidError::conversion_error(format!(
                    "Invalid queen target rank: {}",
                    target_rank
                )))
            }
    ...
    fn decode_bishop_target(&self, from: Square, move_value: u8) -> Result<Square> {
        ...
        if target_square >= 0 && target_square < 64 {
            ...
            Err(ScidError::conversion_error(format!(
                "Bishop move out of bounds: {} -> {}",
                from as u8, target_square
            )))
        }
    }
    ...
    fn decode_knight_target(&self, from: Square, move_value: u8) -> Result<Square> {
        ...
        if target_square >= 0 && target_square < 64 {
            ...
            Err(ScidError::conversion_error(format!(
                "Knight move out of bounds: {} + {} = {}",
                from as u8, square_diffs, target_square
            )))
        }
    }
    ...
    fn get_pawn_promotion_from_move_value(&self, move_value: u8) -> Option<Role> {
        ...
        const PROMOTION_PIECES: [Option<Role>; 16] = [
            ...
            Some(Role::Queen),
            Some(Role::Queen),
            Some(Role::Queen), // 3-5: Queen
            Some(Role::Rook),
            Some(Role::Rook),
            Some(Role::Rook), // 6-8: Rook
            Some(Role::Bishop),
            Some(Role::Bishop),
            Some(Role::Bishop), // 9-11: Bishop
            Some(Role::Knight),
            Some(Role::Knight),
            Some(Role::Knight), // 12-14: Knight
    ...
    fn convert_scid_to_shakmaty_with_diagnostics(
        ...
        if let Err(ref error) = result {
            ...
            eprintln!(
                "   Move #{}: Raw byte 0x{:02X} ({:#010b})",
                move_number, raw_byte, raw_byte
            );
            eprintln!("   Piece: {} | Value: {}", raw_byte >> 4, raw_byte & 15);
            eprintln!(
                "   Piece num: {} | Move value: {}",
                scid_move.piece_num, scid_move.move_value
            );
            ...
            eprintln!(
                ...
                self.current_position
                    .board()
                    .board_fen(shakmaty::Bitboard::EMPTY)
            );
        ...
        if (piece_num as usize) < self.piece_lists[self.to_move as usize].len() {
            ...
            if let Some(piece) = self.current_position.board().piece_at(piece_square) {
                ...
                if piece.role == Role::Pawn && move_value >= 3 {
                    eprintln!(
                        "      ⚠️  COMPLEX PAWN MOVE - value {} indicates promotion",
                        move_value
                    );
                    match move_value {
...
fn extract_promotion_from_scid_move(scid_move: &DecodedMove) -> Result<Role> {
    match &scid_move.interpretation {
        MoveInterpretation::Pawn { promotion, .. } => promotion
            ...
            .ok_or_else(|| ScidError::conversion_error("Invalid promotion piece".to_string())),
        _ => Err(ScidError::conversion_error("Not a pawn move".to_string())),
    }
}
...
mod tests {
    ...
    fn test_position_tracker_creation() {
        ...
        assert_eq!(
            tracker.current_position().board().to_string(),
            Chess::default().board().to_string()
        );
    }
    ...
    fn test_piece_list_initialization() {
        ...
        assert_eq!(
            tracker.get_piece_square(0, Color::White).unwrap(),
            Square::E1
        );

        // Black should have king at piece 0
        assert_eq!(
            tracker.get_piece_square(0, Color::Black).unwrap(),
            Square::E8
        );
    ...
    fn test_complete_scid_move_decoding_pipeline() {
        ...
        assert!(
            result.is_ok(),
            "SCID move pipeline should succeed for valid move"
        );
    ...
    fn test_scid_piece_ordering() {
        ...
        // Piece 1: Queen (D1)
        assert_eq!(
            tracker.get_piece_square(1, Color::White).unwrap(),
            Square::D1
        );
        ...
        assert!(
            rook1 == Square::A1 || rook1 == Square::H1,
            "First rook should be on A1 or H1"
        );
        assert!(
            rook2 == Square::A1 || rook2 == Square::H1,
            "Second rook should be on A1 or H1"
        );
        ...
        assert!(
            bishop1 == Square::C1 || bishop1 == Square::F1,
            "First bishop should be on C1 or F1"
        );
        assert!(
            bishop2 == Square::C1 || bishop2 == Square::F1,
            "Second bishop should be on C1 or F1"
        );
        ...
        assert!(
            knight1 == Square::B1 || knight1 == Square::G1,
            "First knight should be on B1 or G1"
        );
        assert!(
            knight2 == Square::B1 || knight2 == Square::G1,
            "Second knight should be on B1 or G1"
        );
        ...
        for i in 8..16 {
            ...
            assert_eq!(
                ...
                "White pawn {} should be on rank 2",
                i
            );
        }
        ...
        assert_eq!(
            tracker.get_piece_square(1, Color::Black).unwrap(),
            Square::D8
        );
    }
