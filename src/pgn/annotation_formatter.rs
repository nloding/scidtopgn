use crate::formats::sg4::NagProcessor;
...
impl AnnotationFormatter {
    ...
    pub fn format_complete_move(mv: &VariationMove, needs_move_number: bool) -> String {
        ...
        if needs_move_number || mv.is_white_move {
            let move_num_str =
                Self::format_move_number(mv.move_number, mv.is_white_move, needs_move_number);
...
mod tests {
    use super::*;
    use crate::position::{PieceType, ScidMove, Square};
    ...
    fn test_annotation_utilities() {
        ...
        assert!(!AnnotationFormatter::has_annotations(
            &mv_without_annotations
        ));
        assert_eq!(
            AnnotationFormatter::annotation_count(&mv_without_annotations),
            0
        );
    }
