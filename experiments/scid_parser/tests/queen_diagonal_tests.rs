...
fn test_queen_rook_moves_still_work() {
    ...
    assert!(result.is_ok(), "Queen vertical move should still work: {:?}", result);
    ...
    assert!(result.is_ok(), "Queen horizontal move should still work: {:?}", result);
...
fn test_all_diagonal_directions() {
    ...
    for (from_square, expected_to_square, direction) in test_cases.iter() {
        ...
        assert!(result.is_ok(), "Queen diagonal {} move should succeed: {:?}", direction, result);
        assert_eq!(scid_move.to, *expected_to_square, "Queen diagonal {} move target incorrect", direction);
        assert_eq!(stream.position(), 1, "Queen diagonal {} move should consume 1 byte", direction);
    }
}
