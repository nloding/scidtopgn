...
mod tests {
    ...
    use scid_parser::position::decoder::decode_bishop;
    ...
    fn test_bishop_boundary_conditions() {
        ...
        for (square, name) in edge_positions.iter() {
            ...
            for move_value in 0..16 {
                ...
                if result.is_err() {
                    println!("Bishop move {} from {} correctly rejected: {}", 
    ...
    fn test_invalid_bishop_values() {
        ...
        for move_value in invalid_values.iter() {
            let result = decode_bishop(*move_value, &mut scid_move);
            assert!(result.is_err(), "Invalid bishop move value {} should be rejected", move_value);
        }
    }
    ...
    fn test_bishop_scid_reference_moves() {
        ...
        for (move_value, expected_target) in test_cases.iter() {
            ...
            if *expected_target >= 0 && *expected_target <= 63 {
                assert!(result.is_ok(), "Bishop move {} from F1 should succeed", move_value);
                if result.is_ok() {
                    assert_eq!(scid_move.to.0, *expected_target as u8, 
                              "Bishop move {} from F1 target incorrect", move_value);
                }
            } else {
                assert!(result.is_err(), "Bishop move {} from F1 should fail (out of bounds)", move_value);
            }
        }
    }
    ...
    fn test_bishop_exact_scid_algorithm() {
        ...
        for (from_square, move_value, expected_target) in test_cases.iter() {
            ...
            let result = decode_bishop(*move_value, &mut scid_move);
            if *expected_target >= 0 && *expected_target <= 63 {
                assert!(result.is_ok(), "Bishop move {} from {} should succeed", move_value, from_square);
                if result.is_ok() {
                    assert_eq!(scid_move.to.0, *expected_target as u8, 
                ...
                assert!(result.is_err(), "Bishop move {} from {} should fail (out of bounds)", move_value, from_square);
            }
        }
    }
}