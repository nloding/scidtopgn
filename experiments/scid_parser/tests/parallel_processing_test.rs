...
fn test_single_game_parallel_processing() {
    ...
    let results = processor.process_games_parallel(&games_data, Some(&metadata)).unwrap();
    ...
    println!("✅ Single game parallel processing: {} ms", 
...
fn test_multiple_games_parallel_processing() {
    ...
    let metadata: Vec<GameMetadata> = (0..game_count)
        .map(|i| GameMetadata {
            ...
            result: if i % 3 == 0 { "1-0" } else if i % 3 == 1 { "0-1" } else { "1/2-1/2" }.to_string(),
    ...
    let results = processor.process_games_parallel(&games_refs, Some(&metadata)).unwrap();
    ...
    println!("✅ Processed {} games in {:?} ({:.1} games/sec)", 
             game_count, 
             processing_duration,
...
fn test_batch_processing_with_threads() {
    ...
    println!("✅ Batch processing with threading: processed {} games across multiple batches", 
             stats.total_games);
}
...
fn test_performance_statistics() {
    ...
    println!("✅ Performance stats: {:.2} games/sec, avg {:.1}ms per game",
             stats.games_per_second,
...
mod benchmark_tests {
    ...
    fn benchmark_parallel_vs_sequential() {
        ...
        let mut parallel_processor = ParallelGameProcessor::with_config(
            ParallelProcessingConfig {
                thread_count: Some(4),
                batch_size: 100,
                enable_monitoring: false,
        ...
        let parallel_results = parallel_processor.process_games_parallel(&games_refs, None).unwrap();
        let parallel_duration = parallel_start.elapsed();
        
        // Test sequential processing (1 thread)
        let mut sequential_processor = ParallelGameProcessor::with_config(
            ParallelProcessingConfig {
                thread_count: Some(1),
                batch_size: 100,
                enable_monitoring: false,
        ...
        let sequential_results = sequential_processor.process_games_parallel(&games_refs, None).unwrap();
        ...
        println!("✅ Parallel processing ({} threads): {:.2} games/sec ({:?})",
                 4, parallel_stats.games_per_second, parallel_duration);
        println!("✅ Sequential processing (1 thread): {:.2} games/sec ({:?})",
