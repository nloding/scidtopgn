...
mod tests {
    ...
    fn test_position_tracker_single_byte_move() {
        ...
        assert!(result.is_ok(), "Single-byte move should succeed: {:?}", result);
        ...
        match &decoded_move.interpretation {
            MoveInterpretation::Decoded { bytes_consumed, description, from_square, to_square, .. } => {
                ...
                println!("✅ Move: {} -> {}", from_square.as_ref().unwrap(), to_square.as_ref().unwrap());
            },
            _ => panic!("Expected Decoded interpretation, got: {:?}", decoded_move.interpretation),
        }
    }
    ...
    fn test_position_tracker_queen_diagonal_move() {
        ...
        assert!(result.is_ok(), "Queen diagonal move should succeed: {:?}", result);
        let decoded_move = result.unwrap();
        ...
        match &decoded_move.interpretation {
            MoveInterpretation::Decoded { bytes_consumed, description, piece_type, .. } => {
    ...
    fn test_position_tracker_backward_compatibility() {
        ...
        assert!(result.is_ok(), "Backward compatibility should work: {:?}", result);
        let decoded_move = result.unwrap();
    ...
    fn test_position_tracker_multiple_moves_sequence() {
        ...
        match &queen_move.interpretation {
            MoveInterpretation::Decoded { bytes_consumed, .. } => {
                ...
                println!("✅ Queen move consumed {} bytes as expected", bytes_consumed);
            },
            _ => panic!("Expected Decoded interpretation"),
        }
    }
    ...
    fn test_position_tracker_capture_detection() {
        ...
        let decoded_move = result.unwrap();
        match &decoded_move.interpretation {
            MoveInterpretation::Decoded { is_capture, is_promotion, .. } => {
