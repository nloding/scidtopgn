...
#[cfg(test)]
mod tests {
    use scid_parser::position::{ScidMove, Square, PieceType, Color};
    use scid_parser::position::decoder::decode_pawn;
    ...
    fn test_pawn_basic_moves() {
        ...
        for (color, move_value, expected_target, expected_promotion) in test_cases.iter() {
            ...
            assert!(result.is_ok(), "Pawn move {} for {:?} should succeed", move_value, color);
            assert_eq!(scid_move.to.0, *expected_target as u8, 
                      "Pawn move {} for {:?} target incorrect", move_value, color);
            assert_eq!(scid_move.promote, *expected_promotion,
                      "Pawn move {} for {:?} promotion incorrect", move_value, color);
        }
    }
    ...
    fn test_pawn_promotions() {
        ...
        for (color, move_value, expected_target, expected_promotion) in promotion_test_cases.iter() {
            let result = decode_pawn(*move_value, &mut scid_move, *color);
            assert!(result.is_ok(), "Pawn promotion {} for {:?} should succeed", move_value, color);
            assert_eq!(scid_move.to.0, *expected_target as u8, 
                      "Pawn promotion {} for {:?} target incorrect", move_value, color);
            assert_eq!(scid_move.promote, *expected_promotion,
                      "Pawn promotion {} for {:?} promotion piece incorrect", move_value, color);
        }
    }
    ...
    fn test_black_pawn_moves() {
        ...
        for (color, move_value, expected_target, expected_promotion) in test_cases.iter() {
            let result = decode_pawn(*move_value, &mut scid_move, *color);
            assert!(result.is_ok(), "Black pawn move {} should succeed", move_value);
            assert_eq!(scid_move.to.0, *expected_target as u8, 
                      "Black pawn move {} target incorrect: expected {}, got {}", 
                      move_value, *expected_target, scid_move.to.0);
            assert_eq!(scid_move.promote, *expected_promotion,
                      "Black pawn move {} promotion incorrect", move_value);
        }
    }
    ...
    fn test_pawn_boundary_conditions() {
        ...
        for (square, color, name) in edge_positions.iter() {
            ...
            for move_value in 0..16 {
                ...
                if result.is_err() {
                    println!("Pawn move {} from {} ({:?}) correctly rejected: {}", 
                            move_value, name, color, result.unwrap_err());
                }
            }
        }
    }
    ...
    fn test_invalid_pawn_values() {
        ...
        for move_value in invalid_values.iter() {
            ...
            assert!(result.is_err(), "Invalid pawn move value {} should be rejected", move_value);
        }
    }
        ...
        for (from_square, color) in test_cases.iter() {
            ...
            for (move_value, square_diff) in scid_square_diffs.iter().enumerate() {
                ...
                if expected_target >= 0 && expected_target <= 63 {
                    assert!(result.is_ok(), "Pawn move {} from {} ({:?}) should succeed", 
                            move_value, from_square, color);
                    if result.is_ok() {
                        assert_eq!(scid_move.to.0, expected_target as u8, 
                    ...
                    assert!(result.is_err(), "Pawn move {} from {} ({:?}) should fail (out of bounds)", 
                            move_value, from_square, color);
                }
            }
        }
    ...
    fn test_pawn_promotion_pieces() {
        ...
        for (move_value, expected_promo) in expected_promotions.iter().enumerate() {
            ...
            assert_eq!(scid_move.promote, *expected_promo,
