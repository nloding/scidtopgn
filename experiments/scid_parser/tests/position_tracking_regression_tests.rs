...
use scid_parser::si4::{parse_header, parse_game_index};
...
fn test_five_database_position_tracking() {
    ...
    let si4_data = match std::fs::read(format!("{}.si4", test_data_path)) {
        Ok(data) => data,
        Err(e) => {
            println!("⚠️  Could not load SI4 file: {}. Skipping regression test.", e);
    ...
    let sg4_data = match std::fs::read(format!("{}.sg4", test_data_path)) {
        Ok(data) => data,
        Err(e) => {
            println!("⚠️  Could not load SG4 file: {}. Skipping regression test.", e);
            return; // Skip test if file not available
        }
    };
    ...
    let header = match parse_header(&mut si4_cursor) {
        Ok(h) => h,
        Err(e) => {
            println!("⚠️  Could not parse SI4 header: {}. Skipping regression test.", e);
            return; // Skip test if parsing fails
        }
    };
    
    println!("📋 Testing {} games from five.sg4 database", header.num_games);
            ...
            if game_end <= sg4_data.len() {
                ...
                match parse_pgn_tags_with_streaming(game_data) {
                    Ok(parse_result) => {
                        ...
                        println!("🎯 Game {}: {:.1}% success rate ({}/{} moves) - Hash: {:016x}", 
                               game_idx + 1, stats.success_rate, 
    ...
    if games_tested > 0 {
        ...
        println!("   📈 Average success rate across {} games: {:.1}%", games_tested, average_success_rate);
        println!("   📊 Overall success rate: {:.1}% ({}/{} moves)", 
                overall_success_rate, total_successful_moves, total_moves);
        println!("   🎯 Games successfully processed: {}/{}", games_tested, header.num_games);
        ...
        assert!(games_tested > 0, "Should process at least one game successfully");
...
fn test_against_reference_pgn() {
    ...
    let reference_pgn = match std::fs::read_to_string(reference_pgn_path) {
        Ok(content) => content,
        Err(e) => {
            println!("⚠️  Could not load reference PGN: {}. Skipping reference test.", e);
            return; // Skip test if file not available
        }
    };
    
    println!("📖 Loaded reference PGN with {} characters", reference_pgn.len());
    ...
    for (game_idx, reference_game) in reference_games.iter().enumerate() {
        ...
        match parse_scid_game_with_position_tracking(game_idx) {
            Ok(parsed_result) => {
                ...
                println!("   📊 SCID parsing: {:.1}% success ({}/{} moves)", 
                        stats.success_rate, stats.successful_moves, stats.total_moves);
                println!("   📖 Reference game has {} moves", reference_game.move_count);
...
fn test_position_tracking_improvement_metrics() {
    ...
    if let Ok(header) = parse_header(&mut si4_cursor) {
        ...
        for game_idx in 0..std::cmp::min(header.num_games, 3) { // Test first 3 games
            if let Ok(game_index) = parse_game_index(&mut si4_cursor) {
                let game_start = game_index.offset as usize;
                let game_end = game_start + game_index.length as usize;
        ...
        for metric in &improvement_metrics {
            println!("   🎯 Game {}: {:.1}% success ({}/{} moves) - Hash: {:016x}", 
                    metric.game_number, metric.success_rate, 
                    metric.successful_moves, metric.total_moves, metric.position_hash);
        }
        
        if !improvement_metrics.is_empty() {
            let avg_success = improvement_metrics.iter().map(|m| m.success_rate).sum::<f64>() / improvement_metrics.len() as f64;
            let total_moves: usize = improvement_metrics.iter().map(|m| m.total_moves).sum();
            let total_successful: usize = improvement_metrics.iter().map(|m| m.successful_moves).sum();
            
            println!("   📈 Average success rate: {:.1}%", avg_success);
            println!("   📊 Overall success rate: {:.1}% ({}/{})", 
                    (total_successful as f64 / total_moves as f64) * 100.0, total_successful, total_moves);
        }
    }
}
    ...
    for line in pgn_content.lines() {
        ...
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            ...
            if let Some(space_pos) = trimmed.find(' ') {
                ...
                let value = trimmed[space_pos + 1..trimmed.len() - 1].trim_matches('"').to_string();
                current_headers.push((tag, value));
            }
        } else if !in_headers {
            // Parse moves (simplified)
            let moves_in_line: Vec<String> = trimmed.split_whitespace()
            ...
            // Check for game end
            if trimmed.contains("1-0") || trimmed.contains("0-1") || trimmed.contains("1/2-1/2") || trimmed.contains("*") {
                // End of game
                let result = if trimmed.contains("1-0") { "1-0" }
                else if trimmed.contains("0-1") { "0-1" }
                else if trimmed.contains("1/2-1/2") { "1/2-1/2" }
                else { "*" };
...
fn parse_scid_game_with_position_tracking(game_idx: usize) -> Result<scid_parser::sg4::StreamingGameParseState, String> {
    let test_data_path = "/Users/nloding/code/scidtopgn/test/data/five";
    ...
    for _ in 0..=game_idx {
        if let Ok(game_index) = parse_game_index(&mut si4_cursor) {
            if game_idx == 0 || game_idx > 0 { // Parse the game we want
                let game_start = game_index.offset as usize;
                let game_end = game_start + game_index.length as usize;
...
fn validate_move_sequence_compatibility(
    parsed_result: &scid_parser::sg4::StreamingGameParseState, 
    reference_game: &ReferenceGame
) {
    let stats = &parsed_result.position_tracker_stats;
    
    println!("   🔍 Move sequence compatibility check:");
    println!("      📊 SCID: {} total moves, {} successful", stats.total_moves, stats.successful_moves);
    println!("      📖 Reference: {} moves in PGN", reference_game.move_count);
    
    // Basic compatibility check
    if stats.successful_moves > 0 && reference_game.move_count > 0 {
        let compatibility_ratio = (stats.successful_moves as f64 / reference_game.move_count as f64) * 100.0;
        println!("      📈 Compatibility ratio: {:.1}% (SCID successful / Reference total)", compatibility_ratio);
