...
impl OptimizedPositionTracker {
    ...
    pub fn set_max_recent_moves(&mut self, max_moves: usize) {
        ...
        if self.recent_moves.capacity() < max_moves {
            self.recent_moves.reserve(max_moves - self.recent_moves.len());
        }
    }
    ...
    pub fn generate_memory_report(&self) -> String {
        ...
        report.push_str(&format!("   Recent Moves Stored: {}/{}\n", self.recent_moves.len(), self.max_recent_moves));
        
        report.push_str(&format!("\n💾 Memory Usage:\n"));
        report.push_str(&format!("   Position Size: {} bytes\n", std::mem::size_of::<ScidPosition>()));
        report.push_str(&format!("   Recent Moves Size: {} bytes\n", 
                self.recent_moves.capacity() * std::mem::size_of::<ScidMove>()));
        report.push_str(&format!("   Tracker Overhead: {} bytes\n", std::mem::size_of::<Self>()));
        report.push_str(&format!("   Total Estimated: {} bytes ({} KB)\n", 
                self.get_memory_usage_estimate(), self.get_memory_usage_kb()));
        ...
        report.push_str(&format!("\n🎯 Memory Efficiency:\n"));
        report.push_str(&format!("   Storage Ratio: {:.1}% (recent/total moves)\n", efficiency_ratio * 100.0));
...
impl MemoryAnalyzer {
    ...
    pub fn estimate_game_memory_usage(move_count: usize, recent_moves_limit: usize) -> GameMemoryEstimate {
    ...
    pub fn recommend_optimization(move_count: usize, memory_limit_mb: usize) -> OptimizationRecommendation {
        ...
        for &limit in &test_limits {
            ...
            if estimate.total_mb <= memory_limit_mb {
                recommendations.push(format!("✅ Recent moves limit {}: {}MB (within {}MB limit)", 
                    limit, estimate.total_mb, memory_limit_mb));
            } else {
                recommendations.push(format!("❌ Recent moves limit {}: {}MB (exceeds {}MB limit)", 
                    limit, estimate.total_mb, memory_limit_mb));
            }
        }
        ...
        if memory_limit_mb < 1 && compact_estimate < memory_limit_mb * 1024 {
            recommendations.push("💡 Consider CompactPositionTracker for minimal memory usage".to_string());
        }
