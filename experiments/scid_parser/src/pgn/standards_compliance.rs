...
impl PgnStandardsChecker {
    ...
    pub fn format_pgn(&self, pgn_content: &str) -> String {
        ...
        for token in tokens {
            ...
            if current_line_length + token.len() + 1 > self.max_line_length && current_line_length > 0 {
                result.push('\n');
    ...
    fn validate_date_format(&self, date: &str) -> Result<(), String> {
        ...
        if parts[1].len() != 2 || (!parts[1].chars().all(|c| c.is_ascii_digit()) && parts[1] != "??") {
            return Err(format!("Invalid month format: {}", parts[1]));
        }
        
        // Check day format (2 digits or ??)
        if parts[2].len() != 2 || (!parts[2].chars().all(|c| c.is_ascii_digit()) && parts[2] != "??") {
...
"#;
    ...
    fn test_complex_variation_tokenization() {
        ...
        let complex_input = "1.e4 e5 2.Nf3 Nc6 (2...Nf6 3.Nxe5 d6 4.Nf3 Nxe4) 3.Bb5 {Spanish Opening} a6 4.Ba4";
