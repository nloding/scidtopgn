...
fn test_comprehensive_date_extraction() {
    ...
    println!(
        "   Decoded: day={}, month={}, year_raw={}, year_adjusted={}",
    ...
    if std::path::Path::new(pgn_path).exists() {
        let pgn_content = fs::read_to_string(pgn_path).expect("Failed to read PGN file");
        ...
        println!(
            "   Found {} occurrences of '{}' in PGN",
            occurrences, date_tag
        );
        assert_eq!(
            occurrences, 5,
            "Should find exactly 5 games with the target date"
        );
    ...
    for (test_year, test_month, test_day, expected) in edge_cases {
        ...
        assert_eq!(
            result, expected,
            "Edge case ({}.{:02}.{:02}) handling failed",
    ...
    for (result_code, expected_string) in result_cases {
        ...
        assert_eq!(
            test_index.result_string(),
            expected_string,
            "Result string for code {} failed",
            result_code
        );
    }
