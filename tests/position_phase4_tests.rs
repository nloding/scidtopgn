...
mod tests {
    ...
    use scidtopgn::position::{decode_move, Color, PieceType, ScidPosition, Square};
    ...
    fn test_scid_piece_numbering_comprehensive() {
        ...
        assert_eq!(
            pos.piece_at(Square::from_algebraic("e1").unwrap()),
            Some(PieceType::King)
        );
        assert_eq!(
            pos.piece_at(Square::from_algebraic("e2").unwrap()),
            Some(PieceType::Pawn)
        );
    ...
    fn test_cf_byte_decodes_to_e4_comprehensive() {
        ...
        assert_eq!(
            ...
            "Move should start from E2"
        );
        assert_eq!(
            scid_move.to,
            Square::from_algebraic("e4").unwrap(),
            "Move should go to E4"
        );
        assert_eq!(
        ...
        assert_eq!(
            square_diff, 16,
            "Should be 16 squares forward (double push)"
        );
    }
    ...
    fn test_first_five_moves_from_five_pgn() {
        ...
        assert!(
            move1_result.is_ok(),
            "Move 1 (e4) should decode successfully"
        );
        let move1 = move1_result.unwrap();
        assert_eq!(
            move1.interpretation.description().contains("e2-e4"),
            true,
            "Move 1 should be e2-e4, got: {}",
            move1.interpretation.description()
        );
    ...
    fn test_position_updates_correctly() {
        ...
        assert_eq!(
            pos.piece_at(Square::from_algebraic("e4").unwrap()),
            Some(PieceType::Pawn),
    ...
    fn test_error_handling() {
        ...
        assert!(
            ...
            "Should handle piece 15 gracefully"
        );
    ...
    fn test_position_aware_vs_static_difference() {
        ...
        assert_ne!(
            position_aware_result.moving_piece,
            PieceType::Empty,
        ...
        assert_eq!(
            position_aware_result.to_algebraic(&pos),
            "e4",
