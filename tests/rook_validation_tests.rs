...
mod tests {
    ...
    use scid_parser::position::decoder::decode_rook;
    ...
    fn test_rook_vertical_moves() {
        ...
        for target_rank in 0..8 {
            ...
            assert!(result.is_ok(), "Rook vertical move to rank {} should succeed", target_rank);
            
            let expected_square = (target_rank << 3) | 3; // Same file (D), new rank
            assert_eq!(scid_move.to.0, expected_square, 
    ...
    fn test_rook_horizontal_moves() {
        ...
        for target_file in 0..8 {
            ...
            let result = decode_rook(move_value, &mut scid_move);
            assert!(result.is_ok(), "Rook horizontal move to file {} should succeed", target_file);
    ...
    fn test_rook_boundary_positions() {
        ...
        for (square, name) in edge_positions.iter() {
            ...
            for move_value in 0..16 {
                ...
                assert!(result.is_ok(), "Rook move {} from {} should succeed", move_value, name);
            }
        }
    }
    ...
    fn test_invalid_rook_values() {
        ...
        for move_value in invalid_values.iter() {
            let result = decode_rook(*move_value, &mut scid_move);
            assert!(result.is_err(), "Invalid rook move value {} should be rejected", move_value);
        }
    }
    ...
    fn test_rook_scid_reference_moves() {
        ...
        for (move_value, expected_target) in test_cases.iter() {
            let result = decode_rook(*move_value, &mut scid_move);
            assert!(result.is_ok(), "Rook move {} from A1 should succeed", move_value);
            assert_eq!(scid_move.to.0, *expected_target, 
    ...
    fn test_rook_exact_scid_algorithm() {
        ...
        for (from_square, move_value, expected_target) in test_cases.iter() {
            ...
            let result = decode_rook(*move_value, &mut scid_move);
            assert!(result.is_ok(), "Rook move {} from {} should succeed", move_value, from_square);
            assert_eq!(scid_move.to.0, *expected_target, 
