...
#[cfg(test)]
mod tests {
    use scid_parser::position::{ScidPosition, ScidMove, PieceType, Square, Color, ScidByteStream, decode_move_with_stream};
    ...
    fn test_stream_decoder_queen_vertical_move() {
        ...
        assert!(result.is_ok(), "Queen vertical move should succeed: {:?}", result);
        let scid_move = result.unwrap();
    ...
    fn test_stream_decoder_queen_horizontal_move() {
        ...
        assert!(result.is_ok(), "Queen horizontal move should succeed: {:?}", result);
        let scid_move = result.unwrap();
        assert_eq!(scid_move.from, Square(3));  // D1
    ...
    fn test_stream_decoder_invalid_piece_number() {
        ...
        assert!(result.is_ok(), "King null move should succeed: {:?}", result);
    ...
    fn test_stream_decoder_queen_diagonal_buffer_underrun() {
        ...
        assert!(result.is_err(), "Queen diagonal without second byte should fail");
