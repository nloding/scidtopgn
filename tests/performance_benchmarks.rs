...
impl PerformanceBenchmark {
    ...
    fn benchmark_pgn_export_single_game(&mut self) -> bool {
        ...
        let result = self.calculate_benchmark_result(benchmark_name, iterations, durations, total_duration, 0.0);
        
        self.benchmark_results.insert(benchmark_name.to_string(), result.clone());
    ...
    fn benchmark_pgn_standards_validation(&mut self) -> bool {
        ...
        use scid_parser::pgn::PgnStandardsChecker;
    ...
    fn benchmark_pgn_export_multiple_games(&mut self) -> bool {
        ...
        let adjusted_durations: Vec<Duration> = durations.iter()
    ...
    pub fn print_performance_report(&self) {
        ...
        for (name, result) in &self.benchmark_results {
            ...
            println!("    Average Time: {:.2}ms", result.average_duration.as_secs_f64() * 1000.0);
            println!("    Min Time: {:.2}ms", result.min_duration.as_secs_f64() * 1000.0);
            println!("    Max Time: {:.2}ms", result.max_duration.as_secs_f64() * 1000.0);
            ...
            if let Some(target) = target {
                println!("    Target: ≤{:.1}ms, ≥{:.1} ops/sec, ≤{:.1}MB",
        ...
        if success_rate >= 90.0 {
            ...
            println!("  Overall Performance: {:.1}% - ❌ FAIL (minimum 90%)", success_rate);
        }
    }
...
fn create_benchmark_game_index() -> scid_parser::pgn::exporter::SimpleGameIndex {
...
fn create_benchmark_name_database() -> scid_parser::pgn::exporter::SimpleNameDatabase {
    let mut name_db = scid_parser::pgn::exporter::SimpleNameDatabase::new();
    name_db.event_names.insert(1, "Performance Test Tournament".to_string());
...
fn create_multiple_test_games() -> Vec<(scid_parser::pgn::exporter::SimpleGameIndex, String)> {
...
fn create_large_name_database() -> scid_parser::pgn::exporter::SimpleNameDatabase {
    let mut name_db = scid_parser::pgn::exporter::SimpleNameDatabase::new();
...
mod tests {
    ...
    fn test_performance_targets_initialization() {
        ...
        assert!(benchmark.performance_targets.contains_key("pgn_export_single_game"));
        assert!(benchmark.performance_targets.contains_key("pgn_standards_validation"));
    }
    ...
    fn test_run_performance_benchmarks() {
        ...
        let result = benchmark.benchmark_results.get("pgn_standards_validation").unwrap();
