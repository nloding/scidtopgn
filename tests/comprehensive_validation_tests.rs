...
mod tests {
    ...
    use scidtopgn::position::{decode_move, Color, PieceType, ScidPosition};
    ...
    fn test_comprehensive_scid_move_decoding_validation() {
        ...
        assert_eq!(
            cf_move.to_algebraic(&pos),
            "e4",
            "Should generate 'e4' notation"
        );

        println!(
            "   ✅ CF byte (0xCF) correctly decodes to: {}",
            cf_move.to_algebraic(&pos)
        );
        ...
        assert_eq!(
            white_pieces[0].to_algebraic(),
            "e1",
            "Piece 0 should be King at E1"
        );
        assert_eq!(
            white_pieces[12].to_algebraic(),
            "e2",
            "Piece 12 should be E2 pawn"
        );
        assert_eq!(
            white_pieces[1].to_algebraic(),
            "a1",
            "Piece 1 should be A1 rook"
        );
        assert_eq!(
            white_pieces[4].to_algebraic(),
            "d1",
            "Piece 4 should be D1 queen"
        );
        ...
        if let Some(GameElement::Move { raw_byte, .. }) = first_move {
            ...
            assert_eq!(
                decoded.to_algebraic(&pos),
                "e4",
                "First move should decode to e4"
            );
                ...
                if let Ok(decoded_move) = tracker.process_move(*raw_byte, *offset) {
                    ...
                    if total_moves == 1 {
                        ...
                        assert!(
                            desc == "e4"
                                || desc.contains("e2-e4")
                                || (desc.contains("Pawn") && desc.contains("e4")),
                            "First move should be pawn e2-e4 or e4, got: {}",
                            desc
                        );
        ...
        assert!(
            success_rate >= 60.0,
            "Should achieve at least 60% success rate"
        );
        println!(
            "   ✅ Position tracking: {}/{} moves ({:.1}% success)",
        ...
        println!(
            "   ✅ Successfully tested {} piece types",
            piece_tests.len()
        );
    ...
    fn test_position_aware_superiority() {
        ...
        println!(
            "   ✅ Position-aware decoding provides context that static interpretation cannot"
        );
