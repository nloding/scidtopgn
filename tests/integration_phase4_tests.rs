...
mod tests {
    ...
    use scidtopgn::position::{decode_move, ScidPosition};
    use scidtopgn::sg4::{find_game_boundaries, parse_pgn_tags, GameElement};
    use std::path::Path;
    ...
    fn test_data_files_exist() {
        ...
        assert!(
            Path::new(&format!("{}/five.si4", test_data_dir)).exists(),
            "five.si4 should exist"
        );
        assert!(
            Path::new(&format!("{}/five.sg4", test_data_dir)).exists(),
            "five.sg4 should exist"
        );
        assert!(
            Path::new(&format!("{}/five.sn4", test_data_dir)).exists(),
            "five.sn4 should exist"
        );
        assert!(
            Path::new(&format!("{}/five.pgn", test_data_dir)).exists(),
            "five.pgn should exist"
        );
    ...
    fn test_sg4_game_boundaries() {
        ...
        println!(
            "✅ SG4 game boundaries parsed correctly: {} games found",
            games.len()
        );
    }
    ...
    fn test_first_game_elements() {
        ...
        assert!(
            move_count < 200,
            "First game shouldn't have more than 200 moves (sanity check)"
        );
    ...
    fn test_first_move_is_cf_byte() {
        ...
        let first_move = parsed_game
            .elements
            .iter()
            .find(|elem| matches!(elem, GameElement::Move { .. }));

        if let Some(GameElement::Move {
            piece_num,
            move_value,
            raw_byte,
            ..
    ...
    fn test_position_tracking_accuracy() {
        ...
        for (i, element) in parsed_game.elements.iter().enumerate() {
            ...
            } = element
            {
                match tracker.process_move(*raw_byte, *offset) {
                    ...
                    Err(e) => {
                        failed_moves += 1;
                        println!("  Move {}: ❌ {} (byte: 0x{:02X})", i + 1, e, raw_byte);
        ...
        assert!(
            successful_moves > 0,
            "Should successfully decode at least some moves"
        );
        assert!(success_rate >= 50.0, "Success rate should be at least 50%");

        println!(
            "✅ Position tracking test completed with {:.1}% success rate",
            success_rate
        );
    }
    ...
    fn test_match_against_five_pgn() {
        ...
        for (i, (expected, decoded)) in expected_moves.iter().zip(decoded_moves.iter()).enumerate()
        {
            ...
            if i == 0 && *expected == "e4" {
                assert!(
                    decoded.contains("e2-e4") || decoded.contains("e4"),
                    "First move should contain e4 reference, got: {}",
                    decoded
                );
        ...
        assert!(
            !decoded_moves.is_empty(),
            "Should decode at least some moves"
        );
