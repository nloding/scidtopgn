...
use scidtopgn::position::{
    optimization::{
        CompactPositionTracker, MemoryAnalyzer, MovePool,
        ScidOptimizedPositionTracker as OptimizedPositionTracker,
    },
    performance::{
        benchmark_position_tracking, MonitoredPositionTracker, PerformanceMonitor,
        PositionTrackingMetrics,
    },
    PieceType, ScidByteStream, ScidMove, Square,
};
...
fn test_monitored_position_tracker() {
    ...
    assert!(
        metrics.decode_time.as_nanos() > 0,
        "Should record non-zero decode time"
    );
...
fn test_performance_metrics_accuracy() {
    ...
    assert!(
        (metrics.get_success_rate() - 66.67).abs() < 0.1,
        "Success rate should be ~66.67%"
    );
    assert!(
        (metrics.get_average_decode_time_ms() - 6.0).abs() < 0.1,
        "Average decode time should be 6ms"
    );
...
fn test_optimized_position_tracker() {
    ...
    assert!(
        optimized_tracker.get_recent_moves().len() <= 5,
    ...
    assert!(
        memory_usage < 100,
        "Memory usage should be reasonable (< 100KB), got {}KB",
        memory_usage
    );
...
fn test_compact_position_tracker() {
    ...
    assert!(
        stats.memory_footprint < 100,
        "Compact tracker should have minimal memory footprint"
    );
...
fn test_move_pool_efficiency() {
    ...
    assert!(
        pool_stats.available_moves <= 20,
        "Pool should not exceed capacity"
    );
    assert!(
        pool_stats.memory_usage_bytes > 0,
        "Pool should report memory usage"
    );
...
fn test_memory_analyzer() {
    ...
    assert!(
        ...
        "Position should have non-zero size"
    );
    assert!(analysis.move_size > 0, "Move should have non-zero size");
    assert!(
        analysis.compact_tracker_size < analysis.tracker_base_size,
    ...
    println!(
        "   Compact tracker size: {} bytes",
        analysis.compact_tracker_size
    );
    ...
    println!(
        "   Game memory estimate (100 moves, 10 recent): {}KB",
        estimate.total_kb
    );
    ...
    assert!(
        !recommendation.recommendations.is_empty(),
...
fn test_benchmarking_utility() {
    ...
    assert!(
        counter >= 90,
        "Should have performed most operations successfully"
    );
