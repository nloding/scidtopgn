...
fn test_five_database_with_queen_moves() {
    ...
    // Process each game
    for (game_index, (start, end)) in games.iter().enumerate().take(5) { // Test first 5 games
        ...
        println!("   📊 Game {}: {}/{} moves successful ({:.1}%), {} Queen diagonals", 
               game_index + 1, game_successful, game_moves, game_success_rate, game_queen_diagonals);
        
        game_results.push((game_index + 1, game_moves, game_successful, game_success_rate, game_queen_diagonals));
    }
    ...
    for (game_num, moves, successful, rate, queen_diagonals) in game_results {
        println!("   Game {}: {}/{} moves ({:.1}%), {} Queen diagonals", 
    ...
    assert!(total_moves > 0, "Should process at least some moves from the database");
    ...
    if two_byte_moves > 0 {
        println!("✅ Found {} two-byte moves, which suggests multi-byte parsing is working", two_byte_moves);
    } else {
...
fn test_synthetic_queen_diagonal_scenarios() {
    ...
    match decode_move_with_stream(&position, &mut stream_1) {
        ...
        Err(e) => {
            println!("ℹ️  Synthetic move failed (expected due to position setup): {}", e);
...
fn test_parsing_performance_comparison() {
    ...
    assert_eq!(stream.position(), test_data.len(), "Should process all bytes");
