...
use scid_parser::pgn::{EnhancedPgnExporter, ExportOptions, PgnStandardsChecker};
...
1.e4 e5 2.Nf3 Nc6 3.Bb5 a6 4.Ba4 Nf6 5.O-O Be7 1-0"#.to_string(),
    ...
    fn test_pgn_export_standard_headers(&mut self) -> bool {
        ...
        match exporter.export_complete_game(&game_index, moves, Some(&name_db)) {
            Ok(actual_pgn) => {
                ...
                self.test_results.insert(test_name.to_string(), RegressionTestResult {
    ...
    fn test_pgn_standards_compliance(&mut self) -> bool {
        ...
        self.test_results.insert(test_name.to_string(), RegressionTestResult {
            ...
            actual_result: format!("Headers: {}, Lines: {}", 
        ...
        match exporter.export_complete_game(&game_index, moves, Some(&name_db)) {
            Ok(pgn_output) => {
                ...
                self.test_results.insert(test_name.to_string(), RegressionTestResult {
                    ...
                    actual_result: if has_correct_date { "2023.12.25".to_string() } else { "Date not found".to_string() },
                    error_message: if !passed { 
        ...
        match exporter.export_complete_game(&game_index, moves, Some(&name_db)) {
            Ok(pgn_output) => {
                ...
                self.test_results.insert(test_name.to_string(), RegressionTestResult {
                    ...
                    passed,
                    expected_result: expected.clone(),
                    actual_result: if has_correct_eco { "B13".to_string() } else { "ECO not found".to_string() },
                    error_message: if !passed { 
    ...
    fn test_result_formatting(&mut self) -> bool {
        ...
        match exporter.export_complete_game(&game_index, moves, Some(&name_db)) {
            Ok(pgn_output) => {
                let has_correct_result = pgn_output.contains("[Result \"1-0\"]") && pgn_output.ends_with("1-0\n");
                let expected = self.known_good_outputs.get(test_name).unwrap();
                ...
                self.test_results.insert(test_name.to_string(), RegressionTestResult {
                    ...
                    passed,
                    expected_result: expected.clone(),
                    actual_result: if has_correct_result { "1-0".to_string() } else { "Result not found".to_string() },
                    error_message: if !passed { 
    ...
    fn test_line_length_formatting(&mut self) -> bool {
        ...
        let long_input = "1.e4 e5 2.Nf3 Nc6 3.Bb5 a6 4.Ba4 Nf6 5.O-O Be7 6.Re1 b5 7.Bb3 d6 8.c3 O-O 9.h3 Nb8";
        ...
        self.test_results.insert(test_name.to_string(), RegressionTestResult {
            ...
            actual_result: format!("Lines OK: {}, Has breaks: {}, Output: {}", 
    ...
    fn test_header_validation(&mut self) -> bool {
        ...
        self.test_results.insert(test_name.to_string(), RegressionTestResult {
            ...
            actual_result: format!("Valid: {}, Invalid rejected: {}", 
    ...
    fn test_special_character_handling(&mut self) -> bool {
        ...
        name_db.player_names.insert(2, "François Müller".to_string());
        ...
        match exporter.export_complete_game(&game_index, moves, Some(&name_db)) {
            Ok(pgn_output) => {
                ...
                self.test_results.insert(test_name.to_string(), RegressionTestResult {
                    ...
                    actual_result: format!("José: {}, François: {}, Quotes: {}", 
    ...
    pub fn print_regression_report(&self) {
        ...
        if success_rate >= 95.0 {
            ...
            println!("  Overall Success Rate: {:.1}% - ❌ FAIL (minimum 95%)", success_rate);
        }
    }
}
...
mod tests {
    ...
    fn test_known_good_outputs_initialization() {
        ...
        assert!(framework.known_good_outputs.contains_key("pgn_export_standard_headers"));
        assert!(framework.known_good_outputs.contains_key("date_formatting_standard"));
        assert!(framework.known_good_outputs.contains_key("eco_formatting_b13"));
    }
    ...
    fn test_run_all_regression_tests() {
        ...
        assert!(framework.test_results.len() >= 5, "Should run multiple regression tests");
        
        // Verify all tests passed
        let failed_tests: Vec<_> = framework.test_results.values()
            .filter(|r| !r.passed)
            .collect();
        
        if !failed_tests.is_empty() {
            for test in failed_tests {
                println!("Failed test: {} - {}", test.test_name, 
                        test.error_message.as_ref().unwrap_or(&"Unknown".to_string()));
