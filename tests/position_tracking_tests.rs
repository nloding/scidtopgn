...
use scidtopgn::position::decode_move_with_stream;
use scidtopgn::position::performance::MonitoredPositionTracker;
use scidtopgn::position::{PieceType, ScidByteStream, ScidMove, ScidPosition, Square};
...
fn test_error_recovery_mechanisms() {
    ...
    for &byte in &problematic_bytes {
        ...
        assert!(
            result.is_ok(),
            "Should handle byte 0x{:02X} gracefully",
            byte
        );
    ...
    println!(
        "Error recovery test - processed {} bytes",
        problematic_bytes.len()
    );
}
...
fn test_position_tracker_statistics() {
    ...
    let test_bytes = [0xCF, 0x65, 0x2C, 0x99, 0x16, 0xFF, 0x26, 0x00];
    ...
    assert_eq!(
        stats.successful_moves + stats.failed_moves,
        stats.total_moves
    );
...
fn generate_test_pgn_from_parsed_game(
    parse_result: &scidtopgn::sg4::StreamingGameParseState,
) -> String {
