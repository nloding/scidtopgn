...
use scidtopgn::bridge::{GameState, OptimizedPositionTracker, PositionContext};
...
fn test_optimized_tracker_performance_stats() {
    ...
    println!(
        "✅ Optimized tracker processed {} games",
...
fn test_memory_efficient_allocations() {
    ...
    assert!(
        piece_list.capacity() >= 16,
        "Piece list should have pre-allocated capacity"
    );

    let san_list = tracker.get_san_list();
    assert!(
        san_list.capacity() >= 200,
        "SAN list should have pre-allocated capacity"
    );

    println!(
        "✅ Memory optimization: piece_list capacity={}, san_list capacity={}",
        piece_list.capacity(),
        san_list.capacity()
    );
}
...
mod benchmark_tests {
    ...
    fn benchmark_tracker_creation() {
        ...
        assert!(
            ...
            "Tracker creation should be fast"
        );
    }
    ...
    fn benchmark_game_processing() {
        ...
        println!(
            "✅ Processed {} games in {:?} (avg: {:?} per game)",
        ...
        assert!(
            ...
            "Game processing should be efficient"
        );
    }
}
