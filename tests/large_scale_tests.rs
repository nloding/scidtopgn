...
use scid_parser::sn4::parse_sn4_header;
use scid_parser::sg4::parse_pgn_tags;
...
impl LargeScaleTestFramework {
    ...
    pub fn test_databases(&mut self, database_paths: &[&Path]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn test_single_database(&self, database_path: &Path) -> Result<DatabaseTestResult, Box<dyn std::error::Error>> {
        ...
        Ok(DatabaseTestResult {
            database_name: database_path.file_name().unwrap().to_string_lossy().to_string(),
            games_tested,
            games_successful,
            moves_tested,
            moves_successful,
    ...
    fn test_pgn_generation(&self) -> GameTestResult {
        ...
        let simple_game_index = scid_parser::pgn::exporter::SimpleGameIndex {
            event_id: 1,
            site_id: 1,
            white_id: 1,
            black_id: 2,
        ...
        let mut simple_name_db = scid_parser::pgn::exporter::SimpleNameDatabase::new();
        simple_name_db.event_names.insert(1, "Test Event".to_string());
        simple_name_db.site_names.insert(1, "Test Site".to_string());
        simple_name_db.player_names.insert(1, "Player White".to_string());
        simple_name_db.player_names.insert(2, "Player Black".to_string());
    ...
    pub fn get_overall_success_rate(&self) -> (f64, f64) {
        ...
        let game_success_rate = (self.total_games_successful as f64 / self.total_games_tested as f64) * 100.0;
    ...
    pub fn print_report(&self) {
        ...
        println!("  Games Successful: {} ({:.1}%)", self.total_games_successful, game_success_rate);
        println!("  Total Moves Tested: {}", self.total_moves_tested);
        println!("  Moves Successful: {} ({:.1}%)", self.total_moves_successful, move_success_rate);
        println!();
        ...
        for (db_name, result) in &self.results {
            ...
            println!("  {}:", db_name);
            println!("    Games: {}/{} ({:.1}%)", result.games_successful, result.games_tested, db_game_rate);
            println!("    Moves: {}/{} ({:.1}%)", result.moves_successful, result.moves_tested, db_move_rate);
            println!("    Parse Time: {}ms", result.performance_metrics.parse_time_ms);
        ...
        println!("  Game Success Rate >= 90%: {}", if game_success_rate >= 90.0 { "✅ PASS" } else { "❌ FAIL" });
        println!("  Move Success Rate >= 95%: {}", if move_success_rate >= 95.0 { "✅ PASS" } else { "❌ FAIL" });
    }
...
mod tests {
        ...
        if test_db_path.with_extension("si4").exists() {
            ...
            match result {
                Ok(()) => {
                    ...
                    assert!(framework.total_games_tested > 0, "Should test at least some games");
                ...
                Err(e) => {
                    println!("Test failed (this may be expected if test data format differs): {}", e);
                    // Don't fail the test - this is exploratory
                }
            }
        } else {
            println!("Test database not found at {}, skipping large-scale test", test_db_path.display());
        }
    }
    ...
    fn test_success_criteria_validation() {
        ...
        assert!(game_success_rate >= 90.0, 
                "Game success rate should be at least 90%, got {:.1}%", 
                game_success_rate);
        
        assert!(move_success_rate >= 95.0, 
