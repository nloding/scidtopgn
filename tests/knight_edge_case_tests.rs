...
#[cfg(test)]
mod tests {
    use scidtopgn::position::decoder::decode_knight;
    ...
    fn test_invalid_knight_values() {
        ...
        for move_value in invalid_values.iter() {
            ...
            assert!(
                result.is_err(),
                "Invalid knight move value {} should be rejected",
                move_value
            );
        }
    }
    ...
    fn test_knight_squares_from_scid_reference() {
        ...
        for (move_value, expected_target) in test_cases.iter() {
            ...
            if *expected_target >= 0 && *expected_target <= 63 {
                assert!(
                    result.is_ok(),
                    "Knight move {} from G1 should succeed",
                    move_value
                );
                if result.is_ok() {
                    assert_eq!(
                        scid_move.to.0, *expected_target as u8,
                        "Knight move {} from G1 target incorrect",
                        move_value
                    );
                }
            } else {
                assert!(
                    result.is_err(),
                    "Knight move {} from G1 should fail (out of bounds)",
                    move_value
                );
            }
        }
