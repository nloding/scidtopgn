use scidtopgn::bridge::{ChessValidation, ChessValidator, PositionContext, ValidationReport};
use scidtopgn::error::Result;
use scidtopgn::sg4::parse_game_with_position_tracking;
use scidtopgn::{GameMetadata, GameState, ParsedGame, PgnExporter};
use std::fs;
use std::path::Path;
...
fn test_complete_scid_to_pgn_pipeline() -> Result<()> {
    ...
    let game_data = fs::read(&sg4_path).map_err(|e| {
        scidtopgn::ScidError::conversion_error(format!("Failed to read game file: {}", e))
    })?;

    // Parse using position-aware parsing
    let (moves, san_notation) = parse_game_with_position_tracking(&game_data, 1).map_err(|e| {
        scidtopgn::ScidError::conversion_error(format!("Failed to parse game: {}", e))
    })?;

    println!(
        "✅ Successfully parsed {} moves with position tracking",
        moves.len()
    );
    ...
    assert!(
        ...
        "Game contains invalid moves: {:?}",
        validation_report.invalid_moves
    );

    println!(
        "✅ All {} moves passed validation",
        validation_report.total_moves
    );
    ...
    assert!(
        pgn_output.contains("[Event \"Test Event\"]"),
        "PGN missing Event tag"
    );
    assert!(
        pgn_output.contains("[Site \"Test Site\"]"),
        "PGN missing Site tag"
    );
    assert!(
        pgn_output.contains("[Date \"2022.12.19\"]"),
        "PGN missing Date tag"
    );
    assert!(
        pgn_output.contains("[White \"Test White\"]"),
        "PGN missing White tag"
    );
    assert!(
        pgn_output.contains("[Black \"Test Black\"]"),
        "PGN missing Black tag"
    );
    assert!(
        pgn_output.contains("[Result \"*\"]"),
        "PGN missing Result tag"
    );

    // Validate PGN ends with result marker
    assert!(
        pgn_output.trim().ends_with('*'),
        "PGN should end with result marker"
    );
...
fn test_validation_pipeline_with_invalid_moves() -> Result<()> {
    ...
    assert!(
        !validation_report.is_valid,
        "Validation should detect invalid moves"
    );
    assert!(
        !validation_report.invalid_moves.is_empty(),
        "Should have invalid moves"
    );
