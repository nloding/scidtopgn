...
mod tests {
    ...
    fn test_previously_failing_knight_moves() {
        ...
        for (from_square, move_value, should_succeed) in test_cases.iter() {
            ...
            if *should_succeed {
                assert!(
                    result.is_ok(),
                    "Knight move {} from square {} should succeed",
                    move_value,
                    from_square
                );
            } else {
                assert!(
                    result.is_err(),
                    "Knight move {} from square {} should fail (out of bounds)",
                    move_value,
                    from_square
                );
            }
    ...
    fn test_previously_failing_bishop_moves() {
        ...
        for (from_square, move_value, should_succeed) in test_cases.iter() {
            ...
            if *should_succeed {
                assert!(
                    result.is_ok(),
                    "Bishop move {} from square {} should succeed",
                    move_value,
                    from_square
                );
            } else {
                if result.is_err() {
                    println!(
                        "Bishop move {} from square {} correctly rejected: {}",
                        move_value,
                        from_square,
                        result.unwrap_err()
                    );
    ...
    fn test_boundary_conditions_all_pieces() {
        ...
        for (square, name) in boundary_positions.iter() {
            ...
            for move_value in 1..=8 {
                ...
                if result.is_err() {
                    println!(
                        "Knight move {} from {} correctly rejected",
                        move_value, name
                    );
                }
            }
    ...
    fn test_stream_based_multi_byte_moves() {
        ...
        match result {
            Ok(scid_move) => {
                println!(
                    "Queen diagonal move decoded successfully: from {} to {}",
                ...
                assert!(
                    stream.position() >= 1,
                    "Move should consume at least 1 byte"
                );
            }
            Err(e) => {
                println!(
                    "Queen diagonal move failed (expected for some cases): {}",
                    e
                );
            }
        }
    ...
    fn test_edge_case_combinations() {
        ...
        let problematic_bytes = [
            0x65, 0x28, 0x59, 0x06, 0x5C, 0x4D, 0x43, 0x41, 0x70, 0x7C, 0x78, 0x42, 0x40, 0x44,
            0x71, 0x7A, 0x27, 0x49, 0x45, 0x4B, 0x68, 0x11, 0x67, 0x26, 0x0A, 0x34, 0x35, 0x1E,
            0xB1, 0x1B, 0x04, 0x5A, 0x5B, 0x57, 0x72, 0x04, 0xCF, // The famous CF byte
        ];
        ...
        assert!(
            success_rate >= 20.0,
            "Success rate should be at least 20% on edge cases"
        );
