...
fn test_pgn_date_format_validation() {
    ...
    let pgn_content = fs::read_to_string(pgn_path).expect("Failed to read PGN test file");
...
fn test_discovered_pattern_decoding() {
    ...
    let game_index = GameIndex {
        ...
        black_elo: 0,
        flags: 0,
        parsed_flags: scidtopgn::si4::GameFlags {
            start: false,
            promotions: false,
            under_promotions: false,
            delete: false,
            white_opening: false,
            black_opening: false,
            middlegame: false,
...
fn test_scid_date_pattern_variations() {
    ...
    for (expected_year, expected_month, expected_day, year_offset) in test_cases {
        ...
        let encoded =
            (expected_day as u32) | ((expected_month as u32) << 5) | ((year_raw as u32) << 9);
        ...
        assert_eq!(
            decoded_day, expected_day,
            "Day roundtrip failed for {}.{:02}.{:02}",
            expected_year, expected_month, expected_day
        );
        assert_eq!(
            decoded_month, expected_month,
            "Month roundtrip failed for {}.{:02}.{:02}",
            expected_year, expected_month, expected_day
        );
        assert_eq!(
            decoded_year, expected_year,
            "Year roundtrip failed for {}.{:02}.{:02}",
            expected_year, expected_month, expected_day
        );
    }
}
...
fn test_date_edge_cases() {
    ...
    for (year, month, day) in invalid_dates {
        ...
        if year == 0 || year > 2100 {
            ...
            assert!(
                date_string.len() == 10,
                "Date string should be proper length"
            );
